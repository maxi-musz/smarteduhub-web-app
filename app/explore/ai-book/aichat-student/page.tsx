...
export default function AIChatStudentPage() {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.role !== "student") {
      // Redirect to non-student chat page
      router.replace(`/explore/ai-book/aichat-non-student${bookId ? `/${bookId}` : ""}`);
    }
          ...
          <div className="space-y-4">
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              ...
              <p className="text-green-700 mt-2">
                {isStudent
                  ? "This is the student interface for chatting with AI books."
                  : "You should be redirected to the non-student view."}
              </p>
            </div>
            {bookId && (

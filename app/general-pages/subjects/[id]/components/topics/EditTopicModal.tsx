...
import {
  useUpdateTeacherTopic,
  useTeacherTopicById,
  useReorderSingleTopic,
...
export const EditTopicModal = ({
  ...
  const { data: topic, isLoading: isLoadingTopic } = useTeacherTopicById(
    isOpen ? topicId : null
  );
  ...
  // Populate form when topic data is loaded or when modal opens
  useEffect(() => {
    if (isOpen && topic) {
      ...
      const formattedInstructions = topic.instructions ? formatDescription(topic.instructions) : "";
  ...
  const hasChanges = originalValues
    ? title.trim() !== originalValues.title.trim() ||
      ...
      instructions.trim() !== originalValues.instructions.trim() ||
      order !== originalValues.order ||
      isActive !== originalValues.isActive
    : false;

  const updateTopic = useUpdateTeacherTopic();
  const reorderTopic = useReorderSingleTopic();

  // Check if order conflicts with another topic
  const orderChanged = order !== currentOrder;
  const orderConflictWarning = orderChanged && order >= 1 && order <= totalTopics;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Reorder topic if order changed
      if (orderChanged && order !== currentOrder) {
        await reorderTopic.mutateAsync({
          subjectId,
          topicId,
          newPosition: order,
        });
  ...
  const handleClose = () => {
    // Errors will be cleared by the useEffect when isOpen becomes false
    onClose();
  };
              ...
              {orderConflictWarning && (
                <Alert className="mt-2">
                  ...
                  <AlertDescription className="text-xs">
                    <strong>Note:</strong> If you change the order to match another
                    topic&apos;s order, the orders will be interchanged automatically.
                  </AlertDescription>
                </Alert>
              )}
            </div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={
                  ...
                  reorderTopic.isPending ||
                  !isFormValid ||
                  !hasChanges
                }
              >
                {updateTopic.isPending || reorderTopic.isPending ? (
                  <>

...
import { useChapterFileUpload } from "@/hooks/general-materials/use-chapter-file-upload";
import { Loader2, Upload, X, FileText } from "lucide-react";
import { toast } from "sonner";

interface ChapterFileUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  materialId: string;
  chapterId: string;
...
export const ChapterFileUploadModal = ({
  isOpen,
  onClose,
  materialId,
  chapterId,
  chapterTitle,
  onSuccess,
}: ChapterFileUploadModalProps) => {
  const [file, setFile] = useState<File | null>(null);
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [isDragOver, setIsDragOver] = useState(false);

  const uploadFile = useChapterFileUpload();
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-brand-light-accent-1">
              {title.length}/200 characters (defaults to filename if not provided)
            </p>
          </div>

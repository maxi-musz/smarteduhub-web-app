...
import { useUpdateChapter } from "@/hooks/chapters/use-chapters";
...
export const EditChapterModal = ({
  ...
  // Calculate max order (total chapters count)
  const maxOrder = allChapters.length;

  // Find which chapter will be affected by order change
  const affectedChapter = useMemo(() => {
    if (!chapter || !hasChanges || order === originalValues?.order) return null;
    return allChapters.find((c) => c.id !== chapter.id && c.order === order);
  }, [order, chapter, allChapters, hasChanges, originalValues]);
  ...
  const orderError = useMemo(() => {
    ...
    if (order > maxOrder) {
      return `Order cannot exceed ${maxOrder} (total chapters)`;
    }
    return null;
  }, [order, maxOrder]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateChapter.mutateAsync({
        ...
        data: {
          title: title.trim() ? formatChapterTitle(title) : undefined,
      ...
      toast.error(
        error?.message || "Failed to update chapter. Please try again."
      );
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {!orderError && maxOrder > 0 && (
              <p className="text-xs text-brand-light-accent-1">
                Valid range: 1 to {maxOrder} (total: {maxOrder} {maxOrder === 1 ? "chapter" : "chapters"})
              </p>
            )}
            {maxOrder === 0 && (
              <p className="text-xs text-brand-light-accent-1">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm text-yellow-800">
                    ...
                    <ul className="list-disc list-inside space-y-0.5 text-xs">
                      <li>
                        <span className="font-medium">"{chapter.title}"</span> will move to order{" "}
                        <span className="font-medium">{order}</span>
                      </li>
                      <li>
                        <span className="font-medium">"{affectedChapter.title}"</span> will move to
                        order <span className="font-medium">{chapter.order}</span>
                      </li>
                    </ul>
                  </div>
                </div>
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={updateChapter.isPending || !hasChanges || !!orderError}
            >
